...
public class CartService {
     ...
     */
    public boolean processCart() throws SQLException {
        if (isEmpty()) {
            return false;
        }
        ...
        try {
            ...
            if (!purchases.isEmpty()) {
                ...
                commande.setMontantTotal(purchases.stream()
                    .map(CartItem::getSubtotal)
                    .reduce(BigDecimal.ZERO, BigDecimal::add));
                commande.setStatutPaiement(PaymentStatus.EN_ATTENTE);
                ...
                // Get the created commande ID (simplified - in production use RETURN_GENERATED_KEYS)
                // For now we'll update stock directly
