...
import Firma.entities.GestionMarketplace.Categorie;
import Firma.entities.GestionMarketplace.Equipement;
import Firma.entities.GestionMarketplace.Fournisseur;
import Firma.entities.GestionMarketplace.ProductType;
import Firma.services.GestionMarketplace.CategorieService;
import Firma.services.GestionMarketplace.EquipementService;
import Firma.services.GestionMarketplace.FournisseurService;
...
public class EquipementController implements Initializable {
    ...
    private final Firma.services.GestionMarketplace.StockAlertService stockAlertService = Firma.services.GestionMarketplace.StockAlertService.getInstance();
    ...
    void handleAjouter(ActionEvent event) {
        ...
        try {
            ...
            // Vérifier si le stock est en dessous du seuil et envoyer une alerte
            checkStockAndAlert(equipement);
    ...
    void handleModifier(ActionEvent event) {
        ...
        try {
            ...
            // Vérifier si le stock est en dessous du seuil et envoyer une alerte
            checkStockAndAlert(currentEditingEquipement);
     ...
     */
    @FXML
    void handleCheckAllStock(ActionEvent event) {
        Alert confirmAlert = new Alert(Alert.AlertType.CONFIRMATION);
        confirmAlert.setTitle("Vérification du stock");
        confirmAlert.setHeaderText("Vérifier tous les équipements");
        confirmAlert.setContentText("Cette action va vérifier tous les équipements et envoyer des emails d'alerte " +
                "pour ceux dont le stock est en dessous du seuil.\n\nContinuer ?");
        
        Optional<ButtonType> result = confirmAlert.showAndWait();
        if (result.isPresent() && result.get() == ButtonType.OK) {
            // Afficher un indicateur de chargement
            Alert progressAlert = new Alert(Alert.AlertType.INFORMATION);
            progressAlert.setTitle("Vérification en cours");
            progressAlert.setHeaderText(null);
            progressAlert.setContentText("Vérification des stocks en cours...");
            progressAlert.show();
            
            // Exécuter la vérification dans un thread séparé
            new Thread(() -> {
                int alertsSent = stockAlertService.checkAndSendAlerts();
                ...
                javafx.application.Platform.runLater(() -> {
                    ...
                    if (alertsSent > 0) {
                        showAlert(Alert.AlertType.INFORMATION, "Alertes envoyées", 
                            alertsSent + " alerte(s) de stock ont été envoyées à l'administrateur.");
                    } else {
                        showAlert(Alert.AlertType.INFORMATION, "Aucune alerte", 
                            "Tous les équipements ont un stock suffisant.");
                    }
                });
            }).start();
        }
    ...
    /**
     * Vérifie le stock d'un équipement et envoie une alerte si nécessaire
     */
    private void checkStockAndAlert(Equipement equipement) {
        ...
        if (equipement.getQuantiteStock() < equipement.getSeuilAlerte()) {
            // Envoyer l'alerte dans un thread séparé pour ne pas bloquer l'UI
            new Thread(() -> {
                boolean sent = stockAlertService.checkEquipementAndAlert(equipement);
                
                if (sent) {
                    javafx.application.Platform.runLater(() -> {
                        System.out.println("[EquipementController] Alerte de stock envoyée pour: " + equipement.getNom());
                        
                        // Afficher une notification discrète
                        Alert alert = new Alert(Alert.AlertType.INFORMATION);
                        alert.setTitle("Alerte de stock");
                        alert.setHeaderText(null);
                        alert.setContentText("⚠️ Stock faible détecté !\n\n" +
                            "Une alerte a été envoyée à l'administrateur pour le produit:\n" +
                            equipement.getNom() + "\n\n" +
                            "Stock actuel: " + equipement.getQuantiteStock() + " unités\n" +
                            "Seuil d'alerte: " + equipement.getSeuilAlerte() + " unités");
                        alert.show();
                    });
                }
            }).start();
