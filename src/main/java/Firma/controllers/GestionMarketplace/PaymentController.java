...
import Firma.services.GestionMarketplace.PaymentNotificationService;
import Firma.services.GestionMarketplace.StripeService;
import Firma.tools.GestionMarketplace.MapPicker;
...
public class PaymentController implements Initializable {
    ...
    @FXML
    private Label cautionsLabel;
    @FXML
    private Label totalLabel;
    @FXML
    private HBox cautionsRow;
    @FXML
    ...
    private void processOrderAfterPayment(String paymentIntentId) {
        try {
            ...
            Commande commande = cartService.processCart();
            ...
            if (commande != null) {
                ...
                successAlert.setContentText(
                        "Merci pour votre commande.\n\n" +
                                ...
                                + paymentIntentId.substring(0, Math.min(20, paymentIntentId.length())) + "...\n\n" +
                                "Votre commande est en cours de traitement.\n" +
                                "Un email de confirmation vous a été envoyé.");
                successAlert.showAndWait();

                // Send confirmation email in background thread (non-blocking)
                final Commande finalCommande = commande;
                CompletableFuture.runAsync(() -> {
                    boolean emailSent = PaymentNotificationService.getInstance()
                            .processPaymentNotification(finalCommande);
                    if (!emailSent) {
                        System.err.println("⚠️ Email de confirmation non envoyé pour la commande: "
                                + finalCommande.getId());
                    }
                });
