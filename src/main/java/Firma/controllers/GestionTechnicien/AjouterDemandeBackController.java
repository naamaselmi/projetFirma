...
import Firma.services.GestionTechnicien.Demandeservice;
import Firma.services.GestionTechnicien.EmailServiceTechnicien;
...
public class AjouterDemandeBackController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        emailService = new EmailServiceTechnicien();
    ...
    /**
     * Charger les informations du technicien connecté via la session
     */
    private void chargerTechnicienConnecte() {
        try {
            // Récupérer l'utilisateur connecté depuis la session
            Firma.entities.GestionEvenement.Utilisateur userSession =
                    SessionManager.getInstance().getUtilisateur();

            if (userSession != null) {
                // Trouver le technicien dont id_utilisateur = id de l'utilisateur connecté
                technicienConnecte = technicienService.getTechnicienByIdUtilisateur(userSession.getId());
                if (technicienConnecte != null) {
                    idTechnicienConnecte = technicienConnecte.getId_tech();
                }
            }

            // Fallback : si aucun technicien trouvé via session, prendre le premier de la liste
            if (technicienConnecte == null) {
                java.util.List<Technicien> liste = technicienService.getdata();
                if (!liste.isEmpty()) {
                    technicienConnecte = liste.get(0);
                    idTechnicienConnecte = technicienConnecte.getId_tech();
                }
            }
    ...
    private void retournerListeTechnicien() {
        try {
            Parent root = FXMLLoader.load(getClass().getResource("/GestionTechnicien/ListeDesTechniciens.fxml"));
