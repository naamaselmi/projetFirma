...
import Firma.entities.GestionMarketplace.CartItem;
...
public class CartService {
    ...
    public Commande processCart() throws SQLException {
        ...
        try {
            ...
            if (!purchases.isEmpty()) {
                ...
                for (CartItem item : purchases) {
                    if (item.getProduct() instanceof Equipement) {
                        ...
                        // Vérifier le stock et envoyer alerte si nécessaire
                        checkStockAndAlert(eq);
            ...
            // If only rentals (no purchases), create a dummy commande to signal success
            if (createdCommande == null && hasRentals) {
                createdCommande = new Commande();
                createdCommande.setId(-1); // Dummy ID to indicate rental-only transaction
            }
    ...
    private void checkStockAndAlert(Equipement equipement) {
        ...
        if (equipement.getQuantiteStock() < equipement.getSeuilAlerte()) {
            // Envoyer l'alerte dans un thread séparé
            new Thread(() -> {
                StockAlertService alertService = StockAlertService.getInstance();
                boolean sent = alertService.checkEquipementAndAlert(equipement);
                
                if (sent) {
                    System.out.println("[CartService] Alerte de stock envoyée pour: " + equipement.getNom() + 
                        " (Stock: " + equipement.getQuantiteStock() + ", Seuil: " + equipement.getSeuilAlerte() + ")");
                }
            }, "cart-stock-alert").start();
        }
    }
}
